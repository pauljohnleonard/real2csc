import { RealMeasure, RealMusic } from './classes';

let measures: RealMeasure[],
  startRepeatLocation: number,
  lastChord: string,
  endRepeatLocation: number,
...
export type Rule = {
  token: string | RegExp;
  description: string;
  operation?: (match: RegExpMatchArray | null) => void;
};
...
const rules: Rule[] = [
  ...
  },
  {
    ...
    token: /[A-GWp]{1}[\+\-\^\dhob#suadlt]*(\/[A-G][#b]?)?/,
    description: 'Chord',
    operation: pushChordInMeasures,
  },
];
...
function parse(inputString: string) {
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (typeof rule.token === 'string' && inputString.startsWith(rule.token)) {
      if (rule.operation) rule.operation(null);
...
function removeEmptyMeasures(measures: RealMeasure[]) {
  return measures.filter((x: RealMeasure) => x?.length !== 0);
}

export function parser(raw: string): RealMusic {
  resetVars();
  parse(raw);
